...
//        b.position(b.position() + 16);
        ...
        //struct icbtag ICBTag;
        {
            ...
            b.put((byte) 0);
            // File Type Uint8
            b.put(ino.isDirectory() ? UDF_ICBTAG_FILETYPE_DIRECTORY : (ino.isSymLink() ? UDF_ICBTAG_FILETYPE_SYMLINK : UDF_ICBTAG_FILETYPE_BYTESEQ));
...
//        System.err.printf("pad %s %d %d\n", name, b.position(), (b.position() & (4 - 1)));
    ...
    public static long fileData(final BlockSink out, final Inode ino, byte[] buf, long size, MessageDigest md, Console userConsole) throws IOException {
        InputStream in;
        if (userConsole != null) {
            RETRY:
            for (;;) {
